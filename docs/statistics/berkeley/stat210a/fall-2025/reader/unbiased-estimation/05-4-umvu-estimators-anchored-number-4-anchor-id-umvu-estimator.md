---
title: 4 UMVU estimators {.anchored number="4" anchor-id="umvu-estimators"}
...
# 4 UMVU estimators {.anchored number="4" anchor-id="umvu-estimators"}
...
In this section we will combine two key facts from this lecture and last concerning unbiased estimation of any estimand <span class="math inline">\$g(\\theta)\$</span> when we have access to a complete sufficient statistic <span class="math inline">\$T(X)\$</span>.
...
Since <span class="math inline">\$\\text{MSE}(\\theta; \\delta) = \\text{Var}\_\\theta(\\delta(X))\$</span> for any unbiased estimator, and the squared error loss is strictly convex, the foregoing theorem immediately implies the existence of a unique UMVU estimator for any U-estimable <span class="math inline">\$g(\\theta)\$</span>, whenever we have a complete sufficient statistic.
...
[← 3 The Rao-Blackwell Theorem {.anchored number="3" anchor-id="the-rao-blackwell-theorem"}](04-3-the-rao-blackwell-theorem-anchored-number-3-anchor-id-the.md) · [Up: contents](index.md) · [5 Finding the UMVUE {.anchored number="5" anchor-id="finding-the-umvue"} →](06-5-finding-the-umvue-anchored-number-5-anchor-id-finding-the.md)
