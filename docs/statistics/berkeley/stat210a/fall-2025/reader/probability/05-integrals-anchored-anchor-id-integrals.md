---
title: Integrals {.anchored anchor-id="integrals"}
...
# Integrals {.anchored anchor-id="integrals"}
...
For example, we could approximate a function <span class="math inline">\$f\$</span> using a sequence of simple functions <span class="math inline">\$f\_n\$</span> that inscribe “ziggurats” beneath <span class="math inline">\$f\$</span> with vertical steps of size <span class="math inline">\$2^{-n}\$</span>: <span class="math display">\\$$ f\_n(x) = 2^{-n}\\cdot \\lfloor 2^n f(x) \\rfloor = \\sum\_{k=0}^{\\infty} k 2^{-n} 1\_{A\_{n,k}}(x), \\$$</span> where <span class="math display">\\$$ A\_{n,k} = \\left\\{x:\\; f(x) \\in \\left\[k 2^{-n}, (k+1) 2^{-n}\\right)\\right\\}, \\$$</span> as illustrated in the picture below.
...
As a result, we have defined <span class="math inline">\$\\int f\\,d\\mu\$</span> for any function <span class="math inline">\$f\$</span> whose positive and negative parts can both be approximated from below by simple functions. Note that we have left out some important details in this presentation (for example we have not characterized which functions <span class="math inline">\$f\$</span> are nice enough to be approximated well by simple functions) but these details are unimportant for our present purposes. The important thing to know is that to any measure <span class="math inline">\$\\mu\$</span> there corresponds a well-defined integral <span class="math inline">\$\\int \\cdot \\,d\\mu\$</span>, which behaves as we expect it to.
...
[← Measures {.anchored anchor-id="measures"}](04-measures-anchored-anchor-id-measures.md) · [Up: contents](index.md) · [Densities {.anchored anchor-id="densities"} →](06-densities-anchored-anchor-id-densities.md)
