...
To do actual calculations of probabilities and expectations we use the distributions of the random variables involved in that particular calculation. The distribution of a random variable <span class="math inline">\$X(\\omega)\$</span> is implicitly the push-forward measure <span class="math inline">\$Q = \\PP \\circ X^{-1}\$</span>, but we will always directly define the distributions of the random variables themselves rather than working with, or even mentioning, the abstract outcome <span class="math inline">\$\\omega\$</span>.

---

[← Densities {.anchored anchor-id="densities"}](06-densities-anchored-anchor-id-densities.md) · [Up: contents](index.md) · [Conditional probability {.anchored anchor-id="conditional-probability"} →](08-conditional-probability-anchored-anchor-id-conditional-proba.md)
