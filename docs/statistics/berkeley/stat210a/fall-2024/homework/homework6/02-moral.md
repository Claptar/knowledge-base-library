...
**Problem 2** (Soft thresholding). Consider the *soft thresholding operator* with parameter $\lambda \geq 0$, defined as $$\eta_\lambda(x) =
\begin{cases}
  x - \lambda & x > \lambda\\
  0 & |x| \leq \lambda\\
  x + \lambda & x < -\lambda
...
[← Homework6 Part 01 —](01-homework6-part-01.md) · [Up: contents](index.md) · [Moral →](03-moral.md)
