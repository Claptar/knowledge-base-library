...
The LASSO estimator _β_<sup>ˆ</sup> lasso( _λ_ ) is usually sparse which means that most of _β_<sup>ˆ</sup> 2<sup>lasso</sup> ( _λ_ ) _, . . . , β_<sup>ˆ</sup> _n_<sup>lasso</sup> _−_ 1<sup>(</sup><sup>_λ_)</sup> are exactly (up to numerical precision) equal to zero. This implies that _µ_ ˆ<sup>lasso</sup> _t_ ( _λ_ ) is piecewise

3

linear. On the other hand, _β_<sup>ˆ</sup> ridge( _λ_ ) will not be sparse in that all the terms _β_<sup>ˆ</sup> 2<sup>ridge</sup> ( _λ_ ) _, . . . , β_<sup>ˆ</sup> _n_<sup>ridge</sup> _−_ 1<sup>(</sup><sup>_λ_)</sup> will be nonzero (even though they may be small). This gives a smooth appearance to _β_ ˆridge( _λ_ ).
...
**Fact 6.1** (Simple Ridge) **.** _Suppose y is a real number and λ >_ 0 _. Then the minimizer of_
...
**Fact 6.2** (Simple LASSO) **.** _Suppose y is a real number and λ >_ 0 _. Then the minimizer of_
...
[← 5 Alternative Representations](05-5-alternative-representations.md) · [Up: contents](index.md) · [7 Cross-validation for selecting λ →](07-7-cross-validation-for-selecting-λ.md)
