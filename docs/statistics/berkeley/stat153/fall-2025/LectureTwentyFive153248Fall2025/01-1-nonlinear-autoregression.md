...
Note that (4) can also be treated as a linear regression model but the linearity is in terms of the _feature vector rt_ (not in terms of the original covariate _xt_ ). We shall refer to _rt_ as the feature vector at time _t_ ; it is also common to refer to it as the hidden layer output at time _t_ .
