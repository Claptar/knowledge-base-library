---
title: Make a new X matrix with the original bestf we estimated from the training
  data, and
source: https://github.com/berkeley-stat153/spring-2026/blob/c08dd12c146698bb6ea1d0c6887d1898a8d98c6e/public/labs/Lab5.ipynb
source_file: sources/berkeley-stat153/spring-2026/public/labs/Lab5.ipynb
licence: CC BY 4.0
route: notebook
fidelity: lossless
converted: '2026-09-14'
---

# Make a new X matrix with the original bestf we estimated from the training data, and

**Source:** [`public/labs/Lab5.ipynb`](https://github.com/berkeley-stat153/spring-2026/blob/c08dd12c146698bb6ea1d0c6887d1898a8d98c6e/public/labs/Lab5.ipynb) · **Licence:** CC BY 4.0 · Converted 2026-09-14 from `.ipynb` (lossless)

# Make a new X matrix with the original best_f we estimated from the training data, and

---

[← Generate some new data for some new time points in the future $t2$](12-generate-some-new-data-for-some-new-time-points-in-the-futur.md) · [Up: contents](index.md) · [use t2 as our new time observations, then calculate our new estimate of y →](14-use-t2-as-our-new-time-observations-then-calculate-our-new-e.md)
