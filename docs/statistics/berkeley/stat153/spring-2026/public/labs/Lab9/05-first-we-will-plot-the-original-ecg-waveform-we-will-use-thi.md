---
title: First we will plot the original ECG waveform. We will use this to measure
source: https://github.com/berkeley-stat153/spring-2026/blob/c08dd12c146698bb6ea1d0c6887d1898a8d98c6e/public/labs/Lab9.ipynb
source_file: sources/berkeley-stat153/spring-2026/public/labs/Lab9.ipynb
licence: CC BY 4.0
route: notebook
fidelity: lossless
converted: '2026-09-14'
---

# First we will plot the original ECG waveform. We will use this to measure

**Source:** [`public/labs/Lab9.ipynb`](https://github.com/berkeley-stat153/spring-2026/blob/c08dd12c146698bb6ea1d0c6887d1898a8d98c6e/public/labs/Lab9.ipynb) · **Licence:** CC BY 4.0 · Converted 2026-09-14 from `.ipynb` (lossless)

# First we will plot the original ECG waveform. We will use this to measure

---

[← MIT-BIH Normal Sinus Rhythm Database, record 16265](04-mit-bih-normal-sinus-rhythm-database-record-16265.md) · [Up: contents](index.md) · [the heart rate variability by finding peaks in the signal (the R wave - which →](06-the-heart-rate-variability-by-finding-peaks-in-the-signal-th.md)
