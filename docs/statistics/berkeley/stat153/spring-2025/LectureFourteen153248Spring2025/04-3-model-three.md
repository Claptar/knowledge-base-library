...
Another fact that we previously verified is _bn−j_ = _b_<sup>¯</sup> _j_ (here _b_<sup>¯</sup> _j_ denotes the complex conjugate of _bj_ ). Because of this property, the later half of the DFT terms is redundant (as they can be recovered from the first half).
