---
title: 10. Multiple regression
source: https://github.com/GTPB/PSLS20/blob/55acd654e639f1d5297dc0f2e46d8fd6855b5bad/theory/08-MultipleRegression.Rmd
source_file: sources/gtpb-psls20/theory/08-MultipleRegression.Rmd
licence: CC BY 4.0
route: markdown
fidelity: lossless
converted: '2026-09-14'
---

# 10. Multiple regression

**Source:** [`theory/08-MultipleRegression.Rmd`](https://github.com/GTPB/PSLS20/blob/55acd654e639f1d5297dc0f2e46d8fd6855b5bad/theory/08-MultipleRegression.Rmd) · **Licence:** CC BY 4.0 · Converted 2026-09-14 from `.Rmd` (lossless)

Split into 8 sections.

1. [Intro](01-intro.md)
2. [Additive multiple linair model](02-additive-multiple-linair-model.md)
3. [fitted points for droplines to surface](03-fitted-points-for-droplines-to-surface.md)
4. [Inference in multiple linear models](04-inference-in-multiple-linear-models.md)
5. [fitted points for droplines to surface](05-fitted-points-for-droplines-to-surface.md)
6. [fitted points for droplines to surface](06-fitted-points-for-droplines-to-surface.md)
7. [ANOVA Tabel](07-anova-tabel.md)
8. [[Home](https://gtpb.github.io/PSLS20/)](08-home-https-gtpb-github-io-psls20.md)

---

[Up: contents](../../index.md)
