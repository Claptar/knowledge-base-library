---
title: RobustRegression.nb
source: https://github.com/statOmics/SGA2020/blob/ded99ebf046477d39ae1950eed10a4ba901b948d/pages/robustRegression.nb.html
source_file: sources/statomics-sga2020-ghpages/pages/robustRegression.nb.html
licence: CC0-1.0
route: pandoc-html
fidelity: good
converted: '2026-09-14'
---

# RobustRegression.nb

**Source:** [`pages/robustRegression.nb.html`](https://github.com/statOmics/SGA2020/blob/ded99ebf046477d39ae1950eed10a4ba901b948d/pages/robustRegression.nb.html) · **Licence:** CC0-1.0 · Converted 2026-09-14 from `.html` (good)

Split into 8 sections.

1. [Introduction](01-introduction.md)
2. [Robust Regression {#robust-regression .title .toc-ignore}](02-robust-regression-robust-regression-title-toc-ignore.md)
3. [Robust regression in R](03-robust-regression-in-r.md)
4. [add outlier at high leverage point](04-add-outlier-at-high-leverage-point.md)
5. [Implement it yourself](05-implement-it-yourself.md)
6. [perform a weighted regression use lm with weights=w](06-perform-a-weighted-regression-use-lm-with-weights-w.md)
7. [plot results](07-plot-results.md)
8. [repeat this many times](08-repeat-this-many-times.md)

---

[Up: contents](../../index.md)
