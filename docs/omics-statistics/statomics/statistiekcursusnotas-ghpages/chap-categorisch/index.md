---
title: 9.1 Inleiding {#inleiding}
source: https://github.com/statOmics/statistiekCursusNotas/blob/a1c505bbdddd3d602fa9cc629bef7ee772233d14/chap-categorisch.html
source_file: sources/statomics-statistiekcursusnotas-ghpages/chap-categorisch.html
licence: unresolved
route: pandoc-html
fidelity: good
converted: '2026-09-14'
---

# 9.1 Inleiding {#inleiding}

**Source:** [`chap-categorisch.html`](https://github.com/statOmics/statistiekCursusNotas/blob/a1c505bbdddd3d602fa9cc629bef7ee772233d14/chap-categorisch.html) · **Licence:** unresolved · Converted 2026-09-14 from `.html` (good)

Split into 4 sections.

1. [9.1 Inleiding {#inleiding}](01-9-1-inleiding-inleiding.md)
2. [9.2 Toetsen voor een proportie](02-9-2-toetsen-voor-een-proportie.md)
3. [9.3 Toets voor associatie tussen 2 kwalitatieve variabelen](03-9-3-toets-voor-associatie-tussen-2-kwalitatieve-variabelen.md)
4. [9.4 Logistische regressie](04-9-4-logistische-regressie.md)

---

[Up: contents](../index.md)
