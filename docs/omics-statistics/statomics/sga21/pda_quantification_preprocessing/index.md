---
title: 'Statistical Methods for Quantitative MS-based Proteomics: Part I. Preprocessing'
source: https://github.com/statOmics/SGA21/blob/0ad787d4cc2bb2f4636440840a8a923cf6c09839/pda_quantification_preprocessing.Rmd
source_file: sources/statomics-sga21/pda_quantification_preprocessing.Rmd
licence: unresolved
route: markdown
fidelity: lossless
converted: '2026-09-14'
---

# Statistical Methods for Quantitative MS-based Proteomics: Part I. Preprocessing

**Source:** [`pda_quantification_preprocessing.Rmd`](https://github.com/statOmics/SGA21/blob/0ad787d4cc2bb2f4636440840a8a923cf6c09839/pda_quantification_preprocessing.Rmd) · **Licence:** unresolved · Converted 2026-09-14 from `.Rmd` (lossless)

Split into 8 sections.

1. [Introduction](01-introduction.md)
2. [Outline](02-outline.md)
3. [Intro: Challenges in Label-Free Quantitative Proteomics](03-intro-challenges-in-label-free-quantitative-proteomics.md)
4. [Import the data in R](04-import-the-data-in-r.md)
5. [Preprocessing](05-preprocessing.md)
6. [Exercise](06-exercise.md)
7. [Software & code](07-software-code.md)
8. [References](08-references.md)

---

[Up: contents](../index.md)
