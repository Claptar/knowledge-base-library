...
Unfortunately, in my experience, byte compiling doesn’t usually speed things up much.
...
Note that _dnorm()_ can handle matrices and vectors as the observations **and** as the means and sds, so there are multiple ways to do this. Try to figure out the fastest way to do this, amongst

77

the options of looping over the mixture components, looping over the observations, using vectorized operations that recycle the observations, using vectorized operations that recycle the mixture components, etc.
...
oneUpdate <- **function** (A, n, K, theta.old, thresh = 0.1) { theta.old1 <- theta.old Theta.old <- theta.old %*% **t** (theta.old) L.old <- **ll** (Theta.old, A)

78

q <- **array** (0, dim = **c** (n, n, K)) **for** (i **in** 1:n) { **for** (j **in** 1:n) { **for** (z **in** 1:K) { **if** (theta.old[i, z]*theta.old[j, z] == 0){ q[i, j, z] <- 0 } **else** { q[i, j, z] <- theta.old[i, z]*theta.old[j, z] / Theta.old[i, j] } } } } theta.new <- theta.old **for** (z **in** 1:K) { theta.new[,z] <- **rowSums** (A*q[,,z])/ **sqrt** ( **sum** (A*q[,,z])) } Theta.new <- theta.new %*% **t** (theta.new) L.new <- **ll** (Theta.new, A) converge.check <- **abs** (L.new - L.old) < thresh theta.new <- theta.new/ **rowSums** (theta.new) **return** ( **list** (theta = theta.new, loglik = L.new, converged = converge.check)) } _# initialize the parameters at random starting values_ temp <- **matrix** ( **runif** (n*K), n, K) theta.init <- temp/ **rowSums** (temp) _# do single update_ out <- **oneUpdate** (A, n, K, theta.init) _# in the real code, oneUpdate was called repeatedly in a while loop_

_# as part of an iterative optimization to find a maximum likelihood estimator_
...
PIKK <- **function** (n, k) { _## return indices of the sample of size k_ **sort** ( **runif** (n), index.return = TRUE)$ix[1:k] } FYKD <- **function** (n, k) { indices <- **seq_len** (n) **for** (i **in** 1:n) { j = **sample** (i:n, 1) tmp <- indices[i]

80

indices[i] <- indices[j] indices[j] <- tmp } **return** (indices[1:k]) }
...
[← %% is modulo operator - it gives the remainder](41-is-modulo-operator---it-gives-the-remainder.md) · [Up: contents](index.md) · [8 Evaluating memory use →](43-8-evaluating-memory-use.md)
