---
title: Unit 02 — dataTech Part 03 —
...
_## 39 in hexadecimal is '9' ## 0a is a newline (at least in Linux/Mac) ## 3a is ':'_

30

x <- **as.raw** ( **c** ('0x4d','0x6f', '0x6d','0x0a')) _## i.e., "Mom\n" in ascii_ x ## [1] 4d 6f 6d 0a **charToRaw** ('Mom\n:') ## [1] 4d 6f 6d 0a 3a **writeBin** (x, 'tmp.txt') **readLines** ('tmp.txt') ## [1] "Mom" **system** ('ls -l tmp.txt', intern = TRUE) ## [1] "-rw-r--r-- 1 paciorek scfstaff 4 Aug 29 10:57 tmp.txt" **system** ('cat tmp.txt')
...
## [1] "from 5#c\xbfa7lw8lz2nX,%@ [128.32.244.179] by ncpc-email with

33

**table** ( **Encoding** (text)) ## ## unknown ## 6936 _## Option 1_ **Encoding** (text) <- "latin1" tmp <- **substring** (text, 1, 15) tmp[6402] ## [1] "from 5#cÂ¿a7lw8l" _## Option 2_ **load** ('../data/IPs.RData') _# loads in an object named 'text'_ tmp <- **substring** (text, 1, 15)
...
And of course you can always save to an R data file using _save.image()_ (to save all the objects

34

in the workspace or _save()_ to save only some objects. Happily this is platform-independent so can be used to transfer R objects between different OS.
...
## [1] " 12.5000 C" " 37.2343 C" ## [3] "1342434324.8000 C" " 0.0000 C" ## [5] "10000000000.0000 C" city <- "Boston"

**sprintf** ("The temperature in %s was %.4f C.", city, temps[1]) ## [1] "The temperature in Boston was 12.5000 C." **sprintf** ("The temperature in %s was %9.4f C.", city, temps[1]) ## [1] "The temperature in Boston was 12.5000 C."
...
[← Unit 02 — dataTech Part 02 —](02-unit-02-datatech-part-02.md) · [Up: contents](index.md)
