...
f <- **function** (x, plot = TRUE, verbose = FALSE) { result <- x[1]^2/1000 + 4*x[1]*x[2]/1000 + 5*x[2]^2/1000 **if** (verbose) **print** (result) **if** (plot && cnt < 10) { **points** (x[1], x[2], pch = **as.character** (cnt)) **if** (cnt < 10) cnt <<- cnt + 1 **else** cnt <<- 1 **if** ( **interactive** ()) **invisible** ( **readline** (prompt = "Press <Enter> to continue...")) } **else if** (plot) **points** (x[1], x[2]) **return** (result) } **library** (fields) **par** (mfrow = **c** (1,2)) x1s <- **seq** (-5, 10, len = 100); x2s = **seq** (-5, 2, len = 100) fx <- **apply** ( **expand.grid** (x1s, x2s), 1, f, FALSE) cnt <- 1

26

**image.plot** (x1s, x2s, **matrix** ( **log** (fx), 100, 100)) init <- **c** (7, -4) **optim** (init, f, method = "Nelder-Mead", verbose = FALSE) ## $par ## [1] -0.0006112564567 0.0001513538132 ## ## $value ## [1] 1.181103575e-10 ## ## $counts ## function gradient ## 65 NA ## ## $convergence ## [1] 0 ## ## $message ## NULL x1s <- **seq** (-.2, .2, len = 100); x2s = **seq** (-.12, .12, len = 100) fx <- **apply** ( **expand.grid** (x1s, x2s), 1, f, FALSE) cnt <- 1 **image.plot** (x1s, x2s, **matrix** ( **log** (fx), 100, 100)) init <- **c** (-0, 0) **optim** (init, f, method = "Nelder-Mead", verbose = FALSE)
