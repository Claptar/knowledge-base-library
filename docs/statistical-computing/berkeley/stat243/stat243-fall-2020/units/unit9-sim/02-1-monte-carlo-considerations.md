...
The simulation variance is _O_ ( _m_<sup><u>1</u>)becausewehave</sup><sup>_m_2inthedenominatorandasumover</sup> _m_ terms in the numerator. Happily, we are in control of _m_ , so in principle we can reduce the simulation error to as little as we desire. Unhappily, as usual, the standard error goes down with the square root of _m_ .
...
- Uncertainty in our estimate of bias: Var<sup>�</sup> ( _E_<sup>�</sup> ( _β_<sup>ˆ</sup> ).
...
Sometimes the _Yi_ are generated in a dependent fashion (e.g., sequential MC or MCMC), in which case this variance estimator does not hold because the samples are not IID, but the estimator _φ_<sup>ˆ</sup> is still correct.
