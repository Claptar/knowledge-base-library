...
Also note that you can call the C functions that implement the R distribution functions as a library ( _Rmath_ ), so if you’re coding in C or another language, you should be able to make use of the standard functions: _{r,p,q,d}{norm,t,gamma,binom,pois,etc.}_ (as well as a variety of

17

other R math functions, which can be seen in _Rmath.h_ ). Phil Spector has a writeup on this (http://www.stat.berkeley.edu/classes/s243/rmath.html) and material can also be found in the _Writing R Extensions_ manual on CRAN (section 6.16).
...
One can also consider truncating the rectangular region, depending on the features of _f_ . Monahan recommends the ratio of uniforms, particularly a version for discrete distributions (p. 323 of the 2nd edition).

20

---

[← Unit 09 — sim Part 06 —](06-unit-09-sim-part-06.md) · [Up: contents](index.md)
