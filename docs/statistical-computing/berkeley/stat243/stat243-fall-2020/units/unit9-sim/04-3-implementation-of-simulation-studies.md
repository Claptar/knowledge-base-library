...
**require** (fields) thetaLevels <- **c** ("low", "med", "hi") n <- **c** (10, 100, 1000) tVsNorm <- **c** ("t", "norm") levels <- **expand.grid** (thetaLevels, tVsNorm, n) _## example of replicate() -- generate m sets correlated normals_ **set.seed** (1) genFun <- **function** (n, theta = 1){ u <- **rnorm** (n) x <- **runif** (n) Cov <- **exp** (- **rdist** (x)/theta)

9

U <- **chol** (Cov) **return** ( **cbind** (x, **crossprod** (U, u))) } m <- 20 simData <- **replicate** (m, **genFun** (100, 1)) **dim** (simData) _# 100 observations by {x, y} values by 20 replicates_ ## [1] 100 2 20

### **3.2 Analysis and reporting**

Often results are reported simply in tables, but it can be helpful to think through whether a graphical representation is more informative (sometimes it’s not or it’s worse, but in some cases it may be much better). Since you’ll often have a variety of scenarios to display, using trellis plots in ggplot2 via the _facet_wrap_ function will often be a good approach to display how results vary as a function of multiple inputs.
...
of the results. Such information includes estimated accuracy of results, as well as descriptions of pseudorandom-number generators, numerical algorithms, programming languages, and major software components used.”
