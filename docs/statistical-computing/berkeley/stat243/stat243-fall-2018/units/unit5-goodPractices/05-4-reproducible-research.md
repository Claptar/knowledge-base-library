...
- Have a file of code for pre-processing, one or more for analysis, and one for figure/table preparation.
   ...
   - Alternatively, use _knitr_ (or _R Markdown_ or _IPython_ ) for your document preparation.
...
2. You might consider using the UNIX utility _make_ , which is generally used for compiling code, as a tool for reproducible research: if interested, see the tutorial on _Using make for workflows_ for more details.
