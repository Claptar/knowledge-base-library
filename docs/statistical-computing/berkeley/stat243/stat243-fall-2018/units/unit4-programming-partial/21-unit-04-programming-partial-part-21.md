---
title: Unit 04 — programming partial Part 21 —
...
# Unit 04 — programming partial Part 21 —
...
x <- **rnorm** (5) x[3] <- 'hat' # What do you think is going to happen? indices <- **c** (1, 2.73) myVec <- 1:10 myVec[indices] ## [1] 1 2
...
In other languages, converting between different classes is sometimes called casting a variable. Here’s an example we can work through that will help illustrate how type conversions occur behind the scenes in R.

19

n <- 5 df <- **data.frame** ( **rep** ('a', n), **rnorm** (n), **rnorm** (n)) **apply** (df, 1, **function** (x) x[2] + x[3])

**## Error in x[2] + x[3]: non-numeric argument to binary operator** ## why does that not work? **apply** (df[ , 2:3], 1, **function** (x) x[1] + x[2]) ## [1] 0.5606 -0.2061 0.6231 0.7105 -0.0417 ## let's look at apply() to better understand what is happening
...
[← Unit 04 — programming partial Part 20 —](20-unit-04-programming-partial-part-20.md) · [Up: contents](index.md) · [Unit 04 — programming partial Part 22 — →](22-unit-04-programming-partial-part-22.md)
