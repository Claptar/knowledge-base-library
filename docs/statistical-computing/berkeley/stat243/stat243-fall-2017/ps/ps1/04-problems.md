...
4. As preparation for future problem sets, this problem explores embedding R code and output in a PDF or HTML document.

Here is some R code that creates a plot and does a bit of calculation.

2

**hist** (LakeHuron) lowHi <- **c** ( **which.min** (LakeHuron), **which.max** (LakeHuron)) yearExtrema <- **attributes** (LakeHuron)$tsp[1]-1 + lowHi
