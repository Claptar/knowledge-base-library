---
title: 4 Eigendecomposition and SVD
...
If _A_ is positive semi-definite, the eigendecomposition is an SVD. Furthermore, _A_<sup>_⊤_</sup> _A_ = _V D_<sup>2</sup> _V_<sup>_⊤_</sup> and _AA_<sup>_⊤_</sup> = _UD_<sup>2</sup> _U_<sup>_⊤_</sup> , so we can find the eigendecomposition of such matrices using the SVD of

27

_A_ (for _AA_<sup>_⊤_</sup> we need to fill out _U_ to have _n_ columns). Note that the squares of the singular values of _A_ are the eigenvalues of _A_<sup>_⊤_</sup> _A_ and _AA_<sup>_⊤_</sup> .
...
**Computation for large tall-skinny matrices** The SVD can also be generated from a QR decomposition. Take _X_ = _QR_ and then do an SVD on the _R_ matrix to get _X_ = _QUDV_<sup>_⊤_</sup> = _U_<sup>_∗_</sup> _DV_<sup>_⊤_</sup> . This is particularly helpful for the case when _X_ is tall and skinny (suppose _X_ is _n×p_ with _n ≫ p_ ),

28

because we can do the tall-skinny QR, and the resulting SVD on _R_ is easy computationally if _p_ is manageable.

---

[← Unit 09 — linalg Part 07 —](07-unit-09-linalg-part-07.md) · [Up: contents](index.md) · [5 Computation →](09-5-computation.md)
