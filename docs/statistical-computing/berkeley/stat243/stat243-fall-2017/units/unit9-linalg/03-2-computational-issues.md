...
norm2 <- **function** (x) **sqrt** ( **sum** (x^2)) A <- **matrix** ( **c** (10,7,8,7,7,5,6,5,8,6,10,9,7,5,9,10),4)

11

e <- **eigen** (A) b <- **c** (32, 23, 33, 31) bPerturb <- **c** (32.1, 22.9, 33.1, 30.9) x <- **solve** (A, b) xPerturb <- **solve** (A, bPerturb) **norm2** (x - xPerturb) ## [1] 16.39695 **norm2** (b - bPerturb) ## [1] 0.2 **norm2** (x - xPerturb)/ **norm2** (x) ## [1] 8.198475 (e$val[1]/e$val[4])* **norm2** (b - bPerturb)/ **norm2** (b) ## [1] 9.942834
...
where the ’perturbation’ is from the inaccuracy of computer numbers. Our exploration of computer
...
x1 <- 1990:2010 x2 <- x1 - 2000 _# centered_ x3 <- x2/10 _# centered and scaled_ X1 <- **cbind** ( **rep** (1, 21), x1, x1^2) X2 <- **cbind** ( **rep** (1, 21), x2, x2^2) X3 <- **cbind** ( **rep** (1, 21), x3, x3^2) e1 <- **eigen** ( **crossprod** (X1)) e1$values ## [1] 3.360186e+14 7.699100e+02 -3.833498e-08 e2 <- **eigen** ( **crossprod** (X2))

13

e2$values ## [1] 50677.704275 770.000000 9.295725 e3 <- **eigen** ( **crossprod** (X3)) e3$values ## [1] 24.112935 7.700000 1.953665
...
[← 1 Preliminaries](02-1-preliminaries.md) · [Up: contents](index.md) · [3 Matrix factorizations (decompositions) and solving systems of linear equations →](04-3-matrix-factorizations-decompositions-and-solving-systems-o.md)
