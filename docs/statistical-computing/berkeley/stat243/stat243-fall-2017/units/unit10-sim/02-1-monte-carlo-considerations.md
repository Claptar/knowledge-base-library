...
The basic idea is that we often want to estimate _φ ≡ Ef_ ( _h_ ( _X_ )) for _X ∼ f_ . Note that if _h_ is an indicator function, this includes estimation of probabilities, e.g., _p_ = _P_ ( _X ≤ x_ ) = _F_ ( _x_ ) = � _−∞x_<sup>_f_(</sup><sup>_t_)</sup><sup>_dt_=</sup> � _I_ ( _t ≤ x_ ) _f_ ( _t_ ) _dt_ = _Ef_ ( _I_ ( _X ≤ x_ )). We would estimate variances or MSEs by having _h_ involve squared terms.

We get an MC estimate of _φ_ based on an iid sample of a large number of values of _X_ from _f_ :


which is justified by the Law of Large Numbers. The simulation variance of _φ_<sup>ˆ</sup> is Var( _φ_<sup>ˆ</sup> ) = _σ_<sup>2</sup> _/m_ , with _σ_<sup>2</sup> = Var( _h_ ( _X_ )). An estimator of _σ_<sup>2</sup> = _Ef_ (( _h_ ( _X_ ) _− φ_ )<sup>2</sup> ) is _σ_ ˆ<sup>2</sup> = _m_ <u>1</u> _−_ 1 �( _h_ ( _Xi_ ) _− φ_ ˆ)2. So our MC simulation error is based on


Sometimes the _Xi_ are generated in a dependent fashion (e.g., sequential MC or MCMC), in which case this variance estimator does not hold because the samples are not IID, but the estimator

2

_µ_ ˆ is still correct. [As a sidenote, a common misperception with MCMC is that you should thin your chains because of dependence of the samples. This is not correct - the only reason to thin a chain is if you want to save on computer storage or processing.]
