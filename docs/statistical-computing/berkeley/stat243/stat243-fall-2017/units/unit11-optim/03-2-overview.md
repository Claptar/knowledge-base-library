...
In one dimension, minimization is the same as root-finding with the derivative function, since the minimum of a differentiable function can only occur at a point at which the derivative is zero. So with differentiable functions we’ll seek to find _x_ s.t. _f_<sup>_′_</sup> ( _x_ ) = _∇f_ ( _x_ ) = 0. To ensure a minimum, we want that for all _y_ in a neighborhood of _x_<sup>_∗_</sup> , _f_ ( _y_ ) _≤ f_ ( _x_<sup>_∗_</sup> ), or (for twice differentiable functions) _f_<sup>_′′_</sup> ( _x_<sup>_∗_</sup> ) = _∇_<sup>2</sup> _f_ ( _x_<sup>_∗_</sup> ) = _Hf_ ( _x_<sup>_∗_</sup> ) _≥_ 0, i.e., that the Hessian is positive semi-definite.
