...
_## playing around..._ x <- "\xa1 \xa2 \xa3 \xf1 \xf2" **Encoding** (x) <- "latin1" x ## [1] "¡ ¢ £ ñ ò"
...
**## Error: invalid multibyte string at ’<bf>a7lw8’** _## the issue occurs with the 6402th element (found by ## trial and error):_ tmp <- **substring** (text[1:6401], 1, 15) tmp <- **substring** (text[1:6402], 1, 15) **## Error: invalid multibyte string at ’<bf>a7lw8’** text[6402] _# note the Latin-1 character_ ## [1] "from 5#c\xbfa7lw8lz2nX,%@ [128.32.244.179] by ncpc-email with _## Interesting:_ **table** ( **Encoding** (text)) ## ## unknown ## 6936 _## Option 1_ **Encoding** (text) <- "latin1" tmp <- **substring** (text, 1, 15) _## Option 2_ **load** ("../data/IPs.RData") _# loads in an object named 'text'_ tmp <- **substring** (text, 1, 15)

10

#### **## Error: invalid multibyte string at ’<bf>a7lw8’**

text <- **iconv** (text, from = "latin1", to = "UTF-8") tmp <- **substring** (text, 1, 15)

11

---

[← 4 File and string encodings](05-4-file-and-string-encodings.md) · [Up: contents](index.md)
