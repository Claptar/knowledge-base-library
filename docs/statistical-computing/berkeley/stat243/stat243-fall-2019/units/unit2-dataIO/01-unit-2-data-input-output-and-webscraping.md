---
title: 'Unit 2: Data input/output and webscraping'
...
## user system elapsed ## 0.750 0.002 0.752 ## [1] ## [1] 2 ## user system elapsed ## 0.827 0.013 0.840 ## [1] "2008,1,29,2,1938,1935,2308,2257,XE,7676,N11176,150,142,104,11,3,SLC,OKC,866,5,41,0,,0,NA,NA,NA,NA,NA" ## [1] 3 ## user system elapsed ## 0.804 0.000 0.805 ## [1] "2008,1,20,7,1540,1525,1651,1637,OO,5703,N227SW,71,72,58,14,15,SBA,SJC,234,5,8,0,,0,NA,NA,NA,NA,NA" ## [1] 4 ## user system elapsed ## 0.814 0.000 0.815 ## [1] "2008,1,2,3,1313,1250,1443,1425,WN,440,N461WN,150,155,138,18,23,MCO,STL,880,3,9,0,,0,2,0,0,0,16" ## [1] 5 ## user system elapsed ## 0.760 0.001 0.759 ## [1] "2008,1,24,4,1026,1015,1116,1110,MQ,3926,N653AE,50,55,38,6,11,MLI,ORD,139,6,6,0,,0,NA,NA,NA,NA,NA" ## [1] 6 ## user system elapsed ## 0.747 0.000 0.747 ## [1] "2008,1,4,5,1129,1125,1352,1350,AA,1145,N438AA,203,205,187,2,4,ORD,SLC,1249,3,13,0,,0,NA,NA,NA,NA,NA" ## [1] 7 ## user system elapsed ## 0.751 0.008 0.759 ## [1] "2008,1,10,4,716,720,1025,1024,DL,1590,N991DL,129,124,107,1,-4,AUS,ATL,813,6,16,0,,0,NA,NA,NA,NA,NA" ## [1] 8 ## user system elapsed ## 0.771 0.000 0.771 ## [1] "2008,2,15,5,2127,2132,2254,2312,XE,7663,N33182,87,100,71,-18,-5,SLC,ABQ,493,6,10,0,,0,NA,NA,NA,NA,NA" **close** (con)
...
**library** (readr) _## ## Attaching package: ’readr’ ## The following object is masked from ’package:curl’: ## ## parse_date ## I'm violating the rule about absolute paths here!! ## (airline.csv is big enough that I don't want to put it in the ## course repository)_ **setwd** ('~/staff/workshops/r-bootcamp-2018/data') **system.time** (dat <- **read.csv** ('airline.csv', stringsAsFactors = FALSE)) ## user system elapsed ## 3.803 0.118 3.924 **system.time** (dat2 <- **read_csv** ('airline.csv')) _## Parsed with column specification: ## cols( ## .default = col_double(),_

9

_## UniqueCarrier = col_character(), ## TailNum = col_character(), ## Origin = col_character(), ## Dest = col_character(), ## CancellationCode = col_character() ## ) ## See spec(...) for full column specifications._ ## user system elapsed ## 1.137 0.032 1.170
...
**library** (rvest) _# uses xml2 ## Loading required package: xml2 ## ## Attaching package: ’rvest’ ## The following object is masked from ’package:readr’: ## ## guess_encoding_ URL <- "https://en.wikipedia.org/wiki/List_of_countries_and_dependencies_by_population" html <- **read_html** (URL) tbls <- **html_table** ( **html_nodes** (html, "table")) **sapply** (tbls, nrow) ## [1] 240 12 pop <- tbls[[1]] **head** (pop) ## Rank Country(or dependent territory) Population ## 1 1 China[Note 2] 1,398,980,000 ## 2 2 India[Note 3] 1,351,900,000 ## 3 3 United States[Note 4] 329,844,000 ## 4 4 Indonesia 268,074,600 ## 5 5 Brazil 210,398,000 ## 6 6 Pakistan 205,909,000 ## Date % of worldpopulation ## 1 September 5, 2019 18.1% ## 2 September 5, 2019 17.5%

11

## 3 September 5, 2019 4.27% ## 4 July 1, 2019 3.47% ## 5 September 5, 2019 2.72% ## 6 September 5, 2019 2.66% ## Source ## 1 Official population clock ## 2 Official population clock ## 3 Official population clock ## 4 Official annual projection ## 5 Official population clock ## 6 Official population clock
...
URL <- "http://www1.ncdc.noaa.gov/pub/data/ghcn/daily/by_year" _## approach 1: search for elements with href attribute_ links <- **read_html** (URL) %>% **html_nodes** ("[href]") %>% **html_attr** ('href') _## approach 2: search for HTML 'a' tags_ links <- **read_html** (URL) %>% **html_nodes** ("a") %>% **html_attr** ('href') **head** (links, n = 10)
...
More generally, we may want to read an HTML document, parse it into its components (i.e., the HTML elements), and navigate through the tree structure of the HTML. Here we use the _XPath_

12

language to specify elements rather than CSS selectors. XPath can also be used for navigating through XML documents.
...
**library** (xml2) doc <- **read_xml** ("https://api.kivaws.org/v1/loans/newest.xml") data <- **as_list** (doc) **names** (data) ## [1] "response" **names** (data$response) ## [1] "paging" "loans" **length** (data$response$loans) ## [1] 20 data$response$loans[[2]][ **c** ('name', 'activity', 'sector', 'location', 'loan_amount')] ## $name ## $name[[1]] ## [1] "Saybah" ## ## ## $activity ## $activity[[1]] ## [1] "Food" ## ## ## $sector ## $sector[[1]] ## [1] "Food"

15

## ## ## $location ## $location$country_code ## $location$country_code[[1]] ## [1] "LR" ## ## ## $location$country ## $location$country[[1]] ## [1] "Liberia" ## ## ## $location$town ## $location$town[[1]] ## [1] "Paynesville South" ## ## ## $location$geo ## $location$geo$level ## $location$geo$level[[1]] ## [1] "town" ## ## ## $location$geo$pairs ## $location$geo$pairs[[1]] ## [1] "6.266909 -10.711196" ## ## ## $location$geo$type ## $location$geo$type[[1]] ## [1] "point" ## ## ## ##

16

## $loan_amount ## $loan_amount[[1]] ## [1] "350"
...
## [1] "id" ## [2] "name" ## [3] "description" ## [4] "status" ## [5] "funded_amount" ## [6] "basket_amount" ## [7] "image" ## [8] "activity" ## [9] "sector" ## [10] "themes" ## [11] "use" ## [12] "location" ## [13] "partner_id" ## [14] "posted_date" ## [15] "planned_expiration_date" ## [16] "loan_amount" ## [17] "borrower_count" ## [18] "lender_count" ## [19] "bonus_credit_eligibility" ## [20] "tags" **names** (loanInfo[[1]]$location) ## [1] "country_code" "country" "town" ## [4] "geo"

17

_## suppose we only want the country locations of the loans (using XPath)_ **xml_find_all** (loansNode, '//location//country') %>% **xml_text** ()

|##|[1] "Liberia"<br>"Liberia"|"Colombia"|"Cambodia"|
|---|---|---|---|
|##|[5] "Cambodia"<br>"Guatemala"|"Guatemala"|"Ghana"|
|##|[9] "Liberia"<br>"Liberia"|"Liberia"|"Liberia"|
|##|[13] "Colombia"<br>"Colombia"|"Haiti"|"Liberia"|
|##|[17] "Liberia"<br>"Liberia"|"Liberia"|"Liberia"|
|_## _|_or extract the geographic co_|_ordinates_||
|**xml**|**_find_all**(loansNode, '//loca|tion//geo/pa|irs')|


|##|{xml_nodeset (20)}|
|---|---|
|##|[1] <pairs>6.266909 -10.711196</pairs>|
|##|[2] <pairs>6.266909 -10.711196</pairs>|
|##|[3] <pairs>10.917049 -74.799071</pairs>|
|##|[4] <pairs>10.932152 104.798771</pairs>|
|##|[5] <pairs>10.932152 104.798771</pairs>|
|##|[6] <pairs>14.88575 -91.512391</pairs>|
|##|[7] <pairs>15.029964 -91.328847</pairs>|
|##|[8] <pairs>5.579185 -0.109031</pairs>|
|##|[9] <pairs>6.266909 -10.711196</pairs>|
|##|[10] <pairs>6.371247 -10.791544</pairs>|
|##|[11] <pairs>6.266909 -10.711196</pairs>|
|##|[12] <pairs>6.371247 -10.791544</pairs>|
|##|[13] <pairs>11.004107 -74.806981</pairs>|
|##|[14] <pairs>10.917049 -74.799071</pairs>|
|##|[15] <pairs>19.14116 -72.014223</pairs>|
|##|[16] <pairs>6.371247 -10.791544</pairs>|
|##|[17] <pairs>6.266909 -10.711196</pairs>|
|##|[18] <pairs>6.371247 -10.791544</pairs>|
|##|[19] <pairs>6.266909 -10.711196</pairs>|
|##|[20] <pairs>6.371247 -10.791544</pairs>|
...
data <- **fromJSON** ("http://api.kivaws.org/v1/loans/newest.json") **names** (data)

## [1] "paging" "loans"

**class** (data$loans) _# nice!_
...
|## 1|1829379||Nanie||en|
|## 2|1829387||Saybah||en|
|## 3|1828509|Seb|astiÃ¡n Enriqu|e|es, en|
|## 4|1829505||Sy||en|
|## 5|1829518||Suon||en|
|## 6|1829371 Muj|eres Virtuosas|De Pajoc Group||en|
|##|status|funded_amount|basket_amount|imag|e.id|
|## 1|fundraising|0|0|324|4236|
|## 2|fundraising|0|0|324|4254|
|## 3|fundraising|0|0|324|3116|
|## 4|fundraising|0|0|324|4428|
|## 5|fundraising|0|0|324|4447|
|## 6|fundraising|0|0|324|4216|
|##|image.templ|ate_id|activ|ity|sector|
|## 1||1<br>Food|Production/Sa|les|Food|
|## 2||1|F|ood|Food|
|## 3||1|Ret|ail|Retail|
|## 4||1 Personal|Housing Expen|ses|Housing|
|## 5||1 Personal|Housing Expen|ses|Housing|
|## 6||1|Texti|les|Arts|
|##||||them|es|
|## 1|||Conflict|Zon|es|
|## 2|||Conflict|Zon|es|
...
|##|4|Solar, Green, Clean En|ergy,|Earth Day Campaign|
|---|---|---|---|---|
|##|5|Solar, Green, Clean En|ergy,|Earth Day Campaign|
|##|6|||NULL|
|##|||||
|##|1|||to buy more in|
|##|2||||
|##|3|to buy candy products|in gen|eral, gift wrap, decorative balloons, hats,|
|##|4||||
|##|5||||
|##|6||||
|##||location.country_code|locati|on.country|
|##|1|LR||Liberia|
|##|2|LR||Liberia|
|##|3|CO||Colombia|
|##|4|KH||Cambodia|
|##|5|KH||Cambodia|
|##|6|GT||Guatemala|
|##||location|.town|location.geo.level|
|##|1|Paynesville|South|town|
|##|2|Paynesville|South|town|
|##|3|Soledad (AtlÃ¡|ntico)|town|
|##|4||Takeo|town|
|##|5||Takeo|town|
|##|6|Olintepeque, Quetzalte|nango|town|
|##||location.geo.pairs l|ocatio|n.geo.type partner_id|
|##|1|6.266909 -10.711196||point<br>182|
|##|2|6.266909 -10.711196||point<br>182|
|##|3|10.917049 -74.799071||point<br>154|
|##|4|10.932152 104.798771||point<br>106|
|##|5|10.932152 104.798771||point<br>106|
|##|6|14.88575 -91.512391||point<br>55|
|##||posted_date p|lanned|_expiration_date|
|##|1|2019-09-05T16:30:51Z|2019|-10-05T16:30:51Z|
|##|2|2019-09-05T16:30:51Z|2019|-10-05T16:30:51Z|
|##|3|2019-09-05T16:30:49Z|2019|-10-05T16:30:49Z|
|##|4|2019-09-05T16:30:46Z|2019|-10-05T16:30:46Z|


21

## 5 2019-09-05T16:30:46Z 2019-10-05T16:30:46Z ## 6 2019-09-05T16:30:03Z 2019-10-05T16:30:03Z ## loan_amount borrower_count lender_count ## 1 200 1 0 ## 2 350 1 0 ## 3 325 1 0 ## 4 1550 1 0 ## 5 1500 1 0 ## 6 3000 8 0 ## bonus_credit_eligibility tags ## 1 TRUE NULL ## 2 TRUE NULL ## 3 TRUE NULL ## 4 FALSE NULL ## 5 FALSE NULL ## 6 FALSE NULL
...
In this case, we can’t see the full URL that is being used. More generally, rather than looking at the URL associated with a link we may need to view the actual HTTP request sent by our browser to the server. We can do this using features of the browser (e.g., in Firefox see Web Developer -> Network and in Chrome More tools -> Developer tools -> Network). Based on this we can see that an HTTP GET request is being used with a URL such as: http://data.un.org/Handlers/DownloadHandler.ashx?DataFilter=itemCode:526;year:2012,2013,2014,2015,2016,2017&DataMartId=FAO&Format=csv&c=2,4,5,6,7&s=countryName:asc,elementCode:asc,year:desc
...
_## http://data.un.org/Handlers/DownloadHandler.ashx?DataFilter=itemCode:526; ##year:2012,2013,2014,2015,2016,2017&DataMartId=FAO&Format=csv&c=2,4,5,6,7& ##s=countryName:asc,elementCode:asc,year:desc_ itemCode <- 526 baseURL <- "http://data.un.org/Handlers/DownloadHandler.ashx"

23

yrs <- **paste** ( **as.character** (2012:2017), collapse = ",") filter <- **paste0** ("?DataFilter=itemCode:", itemCode, ";year:", yrs) args1 <- "&DataMartId=FAO&Format=csv&c=2,3,4,5,6,7&" args2 <- "s=countryName:asc,elementCode:asc,year:desc" url <- **paste0** (baseURL, filter, args1, args2) _## if the website provided a CSV we could just do this: ## apricots <- read.csv(url) ## but it zips the file_ temp <- **tempfile** () _## give name for a temporary file_ **download.file** (url, temp) dat <- **read.csv** ( **unzip** (temp)) _## using a connection (see Section 2)_ **head** (dat)
...
Let’s see an example of accessing climate model output data from the World Bank. The API is documented by following some links from here: http://datahelpdesk.worldbank.org/knowledgebase. Following that documentation we can download monthly average precipitation predictions for 2080-2099 for the US (ISO3 code ‘USA’) based on global climate model simulations. In this case

26

what the World Bank refers to as the REST-based query is simply constructing a straightforward URL.

times <- **c** (2080, 2099) countryCode <- 'USA' baseURL <- "http://climatedataapi.worldbank.org/climateweb/rest/v1/country" _##" http://climatedataapi.worldbank.org/climateweb/rest/v1/country"_ type <- "mavg" var <- "pr" data <- **read.csv** ( **paste** (baseURL, type, var, times[1], times[2], **paste0** (countryCode, '.csv'), sep = '/'))
...
Often what distinguishes an API from what we’ve discussed in previous sections is that the API documents what information it expects from the user and returns the result in a standard format

27

(e.g., a particular file format rather than producing a webpage). Here we can see the Kiva API, which allows us to construct queries on the Kiva data that we saw some of earlier.
...
[Up: contents](index.md) · [Unit 02 — dataIO Part 02 — →](02-unit-02-dataio-part-02.md)
