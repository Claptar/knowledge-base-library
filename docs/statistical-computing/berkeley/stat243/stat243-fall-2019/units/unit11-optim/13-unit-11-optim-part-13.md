...
2. Accept _θ_<sup>˜</sup> as _θt_ +1 according to the probability min(1 _,_ exp(( _f_ ( _θt_ ) _− f_ ( _θ_<sup>˜</sup> )) _/τj_ ) - i.e., accept if a uniform random deviate is less than that probability. Otherwise set _θt_ +1 = _θt_ . Notice that

31

for larger values of _τj_ the differences between the function values at the two locations are reduced (just like a large standard deviation spreads out a distribution). So the exponentiation smooths out the objective function when _τj_ is large.
...
[← $par ## [1] 0 0 ## ## $value](12-value.md) · [Up: contents](index.md) · [6 Basic optimization in R →](14-6-basic-optimization-in-r.md)
