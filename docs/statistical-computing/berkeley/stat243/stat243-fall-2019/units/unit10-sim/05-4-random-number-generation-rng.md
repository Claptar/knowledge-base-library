...
Given the construction, such sequences are periodic if the subsequence ever reappears, which is of course guaranteed because there is a finite number of possible subsequence values given that all the _uk_ values are remainders of divisions by a fixed number . One key to a good random number generator (RNG) is to have a very long period.
...
[← 3 Implementation of simulation studies](04-3-implementation-of-simulation-studies.md) · [Up: contents](index.md) · [Unit 10 — sim Part 06 — →](06-unit-10-sim-part-06.md)
