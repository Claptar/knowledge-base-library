...
At this point, one complication is that the date-time information on the Wikipedia traffic is embedded in the file names. We’d like that information to be fields in the data files. This is done by running the code in _preprocess_wikipedia.py_ in the Python interface to Spark (pyspark). Note that trying to use multiple nodes and to repartition in various ways caused various errors I was unable to

7

diagnose, but the code as is should work albeit somewhat slowly. The resulting data are available to you in _/global/scratch/paciorek/wikistats_full/dated_ . These are the data you will use for PS6.
...
lines.getNumPartitions() # 16800 (480 input files) for full dataset # note delayed evaluation lines.count() # 9467817626 for full dataset

8

---

[← check files on the HDFS, e.g.: hadoop fs -ls /user/paciorek/data/wikistats/raw](02-check-files-on-the-hdfs-e-g-hadoop-fs--ls-user-paciorek-data.md) · [Up: contents](index.md) · [Unit 08 — bigData Part 04 — →](04-unit-08-bigdata-part-04.md)
