---
title: Unit 05 — programming Part 66 —
...
e2 <- **substitute** ( **substitute** (e, **list** (x = 3)), **list** (e = e1)) **substitute** ( **substitute** (e, **list** (x = 3)), **list** (e = e1)) ## substitute(x + y, list(x = 3)) _## so e1 is substituted as an evaluated object, ## which then allows for substitution for 'x'_ e2 ## substitute(x + y, list(x = 3)) **eval** (e2) ## 3 + y **substitute_q** (e1, **list** (x = 3)) _# from pryr_ ## 3 + y
...
**Challenge** : figure out how a _for_ loop is parsed in R. See how a _for_ loop with one statement within the loop differs from one with two or more statements.

109

We’ll see _expression()_ again when we talk about inserting mathematical notation in plots.

110

---

[← [1] "call" eval (e1) # what should I get? ## [1] 11](65-1-call-eval-e1-what-should-i-get-1-11.md) · [Up: contents](index.md)
