...
mean ## function (x, ...) ## UseMethod("mean") ## <bytecode: 0x5560f58c5580> ## <environment: namespace:base> x <- 0; y <- 0 out <- **mean** (x = **c** (3,7)) _# usual way to pass an argument to a function ## what does the following do?_ out <- **mean** (x <- **c** (3,7)) _# this is allowable, though perhaps not useful_ out <- **mean** (y = **c** (3,7))
...
**## Error in x[2] + x[3]: non-numeric argument to binary operator** _## why does that not work?_ **apply** (df[ , 2:3], 1, **function** (x) x[1] + x[2]) ## [1] 1.5435 -1.2013 -0.0872 0.3313 -2.5114 _## let's look at apply() to better understand what is happening_
...
[← Unit 05 — programming Part 19 —](19-unit-05-programming-part-19.md) · [Up: contents](index.md) · [Unit 05 — programming Part 21 — →](21-unit-05-programming-part-21.md)
