...
As an example of exploiting sparsity, here’s how the _spam_ package in R stores a sparse matrix. Consider the matrix to be row-major and store the non-zero elements in order in a vector called

31

_value_ . Then create a vector called _rowptr_ that stores the position of the first element of each row. Finally, have a vector, _colindices_ that tells the column identity of each element. Here’s an example in the spam package in R:

**require** (spam) mat = **matrix** ( **c** (0,0,1,0,10,0,0,0,100,0, **rep** (0,5),1000, **rep** (0,4)), nrow = 4, byrow mat = **as.spam** (mat) mat@entries ## [1] 1 10 100 1000 mat@rowpointers ## [1] 1 3 4 4 5 mat@colindices ## [1] 3 5 4 1
...
[← 5 Eigendecomposition and SVD](12-5-eigendecomposition-and-svd.md) · [Up: contents](index.md) · [7 Iterative solutions of linear systems (optional) →](14-7-iterative-solutions-of-linear-systems-optional.md)
