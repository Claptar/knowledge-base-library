...
**require** (fields) thetaLevels <- **c** ("low", "med", "hi") n <- **c** (10, 100, 1000) tVsNorm <- **c** ("t", "norm")

9

levels <- **expand.grid** (thetaLevels, tVsNorm, n) _## example of replicate() -- generate m sets correlated normals_ **set.seed** (1) genFun <- **function** (n, theta = 1){ u <- **rnorm** (n) x <- **runif** (n) Cov <- **exp** (- **rdist** (x)/theta) U <- **chol** (Cov) **return** ( **cbind** (x, **crossprod** (U, u))) } m <- 20 simData <- **replicate** (m, **genFun** (100, 1)) **dim** (simData) _# 100 observations by {x, y} values by 20 replicates_ ## [1] 100 2 20
