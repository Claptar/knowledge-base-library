...
- Use active names for functions (e.g., _calcLogLik_ , _calc_logLik_ rather than _logLik_ or _logLikCalc_ ). The idea is that a function in a programming language is like a verb in regular language (a function does something), so use a verb in naming it.
