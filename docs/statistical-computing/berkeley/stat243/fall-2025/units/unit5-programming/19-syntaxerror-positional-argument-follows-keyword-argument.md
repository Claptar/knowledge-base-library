...
One real-world example of using decorators is in setting up functions to run in parallel in `dask`, which we'll discuss in Unit 7. Another is the use of Numba to do [just-in-time (JIT) compilation](24-9-efficiency.md#just-in-time-jit-compilation) of Python code.
